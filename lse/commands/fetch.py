...
def fetch_command(
    trace_id: Optional[str] = typer.Option(
        None, "--trace-id", "-t", help="Fetch a specific trace by ID"
    ),
    project: Optional[str] = typer.Option(None, "--project", "-p", help="Filter by project name"),
    start_date: Optional[str] = typer.Option(
        None, "--start-date", help="Start date for trace filtering (YYYY-MM-DD)"
    ),
    end_date: Optional[str] = typer.Option(
        None, "--end-date", help="End date for trace filtering (YYYY-MM-DD)"
    ),
    limit: Optional[int] = typer.Option(
        None, "--limit", "-l", help="Maximum number of traces to fetch"
    ),
) -> None:
    """
    ...
    if not any([trace_id, project, start_date, end_date]):
        console.print(
            "[yellow]Warning:[/yellow] No filters specified. This will fetch all available traces."
        )
        console.print("Consider using --limit to restrict the number of results.")

    # Log the operation
    logger.info(
        f"Fetch parameters: trace_id={trace_id}, project={project}, "
    ...
    try:
        ...
        result = fetch_traces(
            trace_id=trace_id, project=project, start_date=start_dt, end_date=end_dt, limit=limit
        )
        ...
        if result["status"] == "success":
            ...
            console.print(
                f"[red]✗[/red] Fetch operation failed: {result.get('message', 'Unknown error')}"
            )
            raise typer.Exit(1)
